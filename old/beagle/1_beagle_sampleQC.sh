...
plink --bfile ${INPUT}/${s1} --maf 0.001 --make-bed --out ${OUTPUT}/1.${s1}/${s2}_maf
plink --bfile ${OUTPUT}/1.${s1}/${s2}_maf --recode vcf-iid bgz --out ${OUTPUT}/1.${s1}/${s2}_maf


zcat ${OUTPUT}/1.${s1}/${s2}_maf.vcf.gz | awk '$4 != "-" && $5 != "-" && $4 != "I" && $5 != "I" && $4 != "D" && $5 != "D" && $4 != "0" && $5 != "0" && $4 != "N" && $5 != "N" && $4 != "." && $5 != "." '  | gzip > ${OUTPUT}/1.${s1}/${s2}_maf_noINDEL.vcf.gz

bcftools norm -d all -O z -o ${OUTPUT}/1.${s1}/${s2}_maf_noINDEL_rmDupID.vcf.gz ${OUTPUT}/1.${s1}/${s2}_maf_noINDEL.vcf.gz 
...
for chr in $(seq 1 22)
do
plink --bfile ${OUTPUT}/1.${s1}/${s2}_maf_noINDEL_rmDupID_noliftover.autosomal --keep-allele-order --chr $chr --double-id --recode vcf-iid bgz --out ${OUTPUT}/1.${s1}/CHR${chr}_${s2}_noIndel_rmDupID
bcftools index ${OUTPUT}/1.${s1}/CHR${chr}_${s2}_noIndel_rmDupID.vcf.gz
done
